    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Catalogo delle prestazioni sanitarie dell'ospedale</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center rounded-xl border border-dashed border-gray-300 py-16 text-gray-400">
          ...
          <button onClick={apriNuova} className="mt-4 rounded-lg bg-indigo-700 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-800">
            Aggiungi la prima
          </button>
        </div>
      ) : (
              ...
              <div className="space-y-2">
                {lista.filter((p) => p.reparto === reparto).map((p) => (
                  <div key={p.id} className={`rounded-xl border bg-white p-4 flex items-center gap-4 ${p.attiva ? "border-gray-200" : "border-gray-100 opacity-60"}`}>
                    <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg ${p.attiva ? "bg-indigo-50" : "bg-gray-50"}`}>
                      <Stethoscope className={`h-5 w-5 ${p.attiva ? "text-indigo-600" : "text-gray-400"}`} />
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <button onClick={() => apriModifica(p)} className="p-1.5 text-gray-400 hover:text-indigo-700 hover:bg-indigo-50 rounded">
  ...
  return (
    <div>
      ...
      <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder}
        ...
        className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-50 disabled:text-gray-400"
      />
    </div>
  );
}
